...
export default class OpenFilesPaneView {
	...
	updateTitle(item, siblings=true, useLongTitle=false) {
		...
		if (siblings) {
			for (let i = 0; i < this.entries.length; i++) {
				...
				if (entry.item !== item && entry.item.getTitle && entry.item.getTitle() === title) {
					useLongTitle = true;
  ...
  removeEntry(item) {
    ...
    if (index >= 0) {
      let el = document.getElementById(this.entries[index].elementId)
      this.pane.destroyItem(item);
      this.entries.splice(index, 1);
      this.entries.map((entry) => this.updateTitle(entry.item));
      let animationDuration = atom.config.get('open-files.animationDuration')
