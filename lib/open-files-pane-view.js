...
export default class OpenFilesPaneView {
	constructor() {
		...
		atom.config.observe('open-files-extended.collapsable', collapsable => {
			if (collapsable) {
	...
	setPane(pane, addIconToElement) {
      ...
      var itemPath = ''
			...
			let sortOrder = getSortOrder(atom.config.get('open-files-extended.sortOrder'))
			...
			this.element.classList.remove('open-files-hidden');
		}));
	...
	removeEntry(item) {
		...
		if (this.items.length == 0) {
				this.element.classList.add('open-files-hidden');
		}
